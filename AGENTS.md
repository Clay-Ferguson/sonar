...
- `config.py` — YAML config (`Settings`, `load_settings`/`save_settings`) and the accessors the rest of the app reads **at the moment of use** (`search_globs`, `search_prune_args`, `search_depth`, `search_fuzzy`, `open_command`), so saved changes need no restart. `active_patterns()` is the one place the `use_included`/`use_excluded` switches are read.
...
- `search.py` — `SearchRunner` (`QProcess` streaming hits), argv builders for content search (`build_argv`), per-file match spans (`build_match_argv`/`match_spans`, synchronous), and name search (`build_name_argv`, `name_terms`). `literal_query_term()` reduces a query to one string for PDF search.
